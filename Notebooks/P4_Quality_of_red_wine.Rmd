...
ggpairs(rw,columns=c(13,3,9,10,12), title = "Pair plot", params = c(shape = I('.'), outlier.shape = I('.')),corSize=8) + ggplot2::theme_bw()
```
...
ggplot(aes(x=quality,y=residual.sugar),data=rw) + 
  geom_point(position="jitter",alpha = 0.7,color=I("Red"))
```
...
ggplot(aes(x=quality,y=alcohol),data=rw) + 
  geom_point(position="jitter",alpha = 0.7,color=I("Blue"))
...
ggplot(aes(x=quality,y=chlorides),data=rw) + 
  geom_point(position="jitter",alpha = 0.7,color=I("Purple"))
...
ggplot(aes(x=quality,y=pH),data=rw) + 
  geom_point(position="jitter",alpha = 0.7,color=I("Orange"))
...
ggplot(aes(x=quality,y=sulphates),data=rw) + 
  geom_point(position="jitter",alpha = 0.7,color=I("Black"))
```
...
ggplot(aes(x=citric.acid,y=fixed.acidity,color = quality),data=rw) + 
  ...
  stat_smooth(method="lm",se = FALSE,size=1,color = "green") + 
  stat_smooth(method="lm", formula= y ~ poly(x,2) ,se = FALSE,color = "orange",size = 1) + 
  scale_x_continuous(limits=c(0,0.75),breaks=seq(0,0.75,0.05))
```


Here's a look at a linear (green) and a quadratic (orange) trendline.
...
```{r echo=FALSE}
ggplot(aes(x=residual.sugar,y=density),data=subset(rw, residual.sugar<quantile(residual.sugar,0.90))) + 
  geom_point(position = "jitter")
```
...
ggplot(aes(x=sulphates,y=alcohol,color= factor(quality)),data=rw) + 
  ...
  scale_x_continuous(limits = c(0.3,1.4),breaks = seq(0.3,1.4,0.1)) + 
...
ggplot(aes(x=alcohol,color = factor(round_any(sulphates,0.2))),data=rw) + 
  ...
  facet_wrap(factor('quality')) + scale_fill_brewer(palette = "Set1")
```
...
ggplot(aes(x=citric.acid,y=fixed.acidity,color= factor(round_any(volatile.acidity,0.2))),data=rw) + 
  ...
  scale_y_continuous(limits= c(6,16),breaks = seq(6,16,1)) + 
  scale_colour_brewer(palette="Set1")
```
...
ggplot(data=rw) + 
  geom_histogram(aes(x=factor(predRounded),binwidth=1),fill = "black",alpha = 0.5) +
  geom_histogram(aes(x=factor(quality)),binwidth = 1,fill = "red",alpha = 0.5) + 
  xlab("Quality (Actual red, predicted black)") 
...
This doesn't look that bad, but remember that the distribution is already pretty even. Let's have a look at how the predictor quality = 5 performs.
...
ggplot(data=rw) + 
  geom_histogram(aes(x=factor(diffive)),fill="blue",alpha=0.8)+
  geom_histogram(aes(x=factor(diff)),fill="red",alpha=0.8)+
  xlab("Predictor in red, 5 in blue")
```

As we can see, the difference isn't huge. 
...
ggplot(aes(x=citric.acid,y=fixed.acidity,color=quality),data=rw) + 
  geom_point(alpha = 0.7) + 
  stat_smooth(method="lm", formula= y ~ poly(x,2),color = "red",size = 1,se = FALSE) + 
  scale_x_continuous(limits=c(0,0.75),breaks=seq(0,0.75,0.1)) + 
...
### Description One
The relationship between fixed acidity and citric acid seems to follow some sort of quadratic polynomial.

### Plot Two
```{r echo=FALSE, Plot_Two,warnings=FALSE}
ggplot(aes(x=factor(quality),y=alcohol),data=rw) + 
  geom_boxplot(outlier.shape = NA,fill = "grey") +
  geom_point(position = "jitter",alpha = 0.25,color = "red") + 
  ylab("Alcohol (% by volume)") + 
  xlab("Quality") + ggtitle("Alcohol versus quality")
```

### Description Two
All low quality wines have low alcohol levels. All high quality wines have high alcohol levels, increasing from quality five thru 8. 

### Plot Three
```{r echo=FALSE, Plot_Three,warnings=FALSE}
ggplot(aes(x=sulphates,color = factor(quality)),data=rw) + 
  geom_density(size=1.1,alpha = 0.7) + 
  scale_fill_brewer(palette = "Set1") + 
  scale_x_continuous(name = "Sulphates (g/dm^3)", limits = c(0.2,1.3),breaks = seq(0.2,1.3,0.1)) + 
  scale_colour_brewer(palette="Set1") + 
  ggtitle("Sulphate per quality") + ylab("Density")
```

### Description Three
This plot shows how the densities of the sulphate curves for each of the different qualities look. Higher quality wines have more sulphate, but also somewhat flatter curves.
